...
const Grid = React.forwardRef((props: GridData, ref) => {
  ...
  const [perPageItem, setPerPageItem] = useState(props.perPageItem ? props.perPageItem : 10);
  ...
  useEffect(() => {
    if (props.onPageChange) {
      ...
      props.onPageChange(currentPage, isAscending, key, startDate, endDate, filters, dateFilter);
    }
  }, [currentPage]);
  ...
  const updatecheckValue = (index: number) => {
    ...
    setCurrentPage(1)
    props.onSort(1, isAscending, key, startDate, endDate, tempFilter, dateFilter);
  };
  ...
  const sorting = (index: number, asc: boolean, key: any) => {
    ...
    setIsAscending(!asc);
    setKey(key);
    props.onSort(currentPage, !asc, key, startDate, endDate, filters, dateFilter);
  };
  ...
  const onDateFiltering = () => {
    ...
    if (value === true) {
      ...
      setCurrentPage(1)
      props.onSort(1, isAscending, key, startDate, endDate, filters, dateFilter);
    } else {
  ...
  return (
    <div className="grid-div" data-testid="comp-sawin-table">
      ...
      >
        <table className="grid-table" id={props.gridId ? props.gridId : "resizable"}>
          <thead>
            <tr>
              {headers.map((header: GridHeader, i) =>
                header.isShow === false ? (
                  ...
                  >
                    ...
                    {header.isShowCheckBox && <Form.Group className="mb-1">
                      <Form.Check
                        type="checkbox"
            ...
            {props.ShowLoader === true || data.length == 0 ? (
              ...
              data.map((row: GridRow, i) => (
                <tr
                  ...
                  onDoubleClick={() => props.doubleClick && props.doubleClick(i, row)}
                ...
                >
                  {headers.map((header: GridHeader, j) =>
                    header.isShow === false ? (
                      ...
                      <td
                        onClick={() => header.isNotAllowClick ? "" : onSelectRow(i, row)}
                        key={"row_" + i + "_" + j}
                        className={
      ...
      {props.count > 0 ? (
        <div className="d-flex flex-row justify-content-between pb-3 align-items-center pagination-row">
          <div className="d-flex col-6 align-items-center">
            <Pagination
              className=" "
              changePage={(page: number) => { setCurrentPage(page); setStyledRow(-1); }}
              totalItems={props.count}
              itemsCountPerPage={perPageItem}
              changeCurrentPage={currentPage}
            />
            {/* <div className="col-3 position-relative show-record-rows">
              <SawinSelect
                options={pageOption}
                value="pageCount"
                selected={10}
